...
export const Success = ({ requestedLeaves }) => {
  ...
  return <li className="dropdown" id="header-notification-dropdown">
    <a href="#" className="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false">
      <span className="fa fa-bell-o"></span>
      <span className="label label-info notification-badge">{length}</span>
    </a>
    <ul className="dropdown-menu" role="menu">
      <li className="dropdown-header"><Link to={routes.requests()}>{length} leave requests to process</Link></li>
    </ul>
  </li>
}
